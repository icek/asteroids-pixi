...
export class SpaceshipDeathView extends PIXI.Container implements Animatable {
  ...
  constructor() {
    ...
    this.shape1.moveTo(10,0).beginFill(0xffffff).lineTo(-7, 7).lineTo(-4, 0).lineTo(10,0).endFill();
    this.addChild(this.shape1);

    this.shape2 = new PIXI.Graphics();
    this.shape2.moveTo(10,0).beginFill(0xffffff).lineTo(-7, -7).lineTo(-4, 0).lineTo(10, 0).endFill();
  ...
  public animate(time:number):void {
    ...
    shape1.setTransform(shape1.x + this.vel1x * time, shape1.y + this.vel1y * time, 1, 1, shape1.rotation + this.rot1 * time);
    shape2.setTransform(shape2.x + this.vel2x * time, shape2.y + this.vel2y * time, 1, 1, shape2.rotation + this.rot2 * time);
  }
}
